...
def get_train_and_test_dataset(dataset_name):
    if dataset_name == 'imagenette':
        ...
        train_ds = torchvision.datasets.CIFAR100(root=DATA_PATH, train=True, transform=get_benign_transform(dataset_name), download=True)
        test_ds = torchvision.datasets.CIFAR100(root=DATA_PATH, train=False, transform=get_benign_transform(dataset_name, train=False), download=True)
    elif dataset_name == 'gtsrb':
